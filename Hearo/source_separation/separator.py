...
def unified_masking_strategy(Xmel: torch.Tensor, w_bar: torch.Tensor, omega: torch.Tensor, 
    ...
    if is_weak_sound:
        # 작은 소리일 때 적당히 보수적인 마스킹 적용 (너무 강하지 않게)
        soft_time_mask = torch.sigmoid(MASK_SIGMOID_SLOPE * 0.9 * (cos_squared - MASK_SIGMOID_CENTER * 1.1))  # 적당히 보수적
    else:
        # 일반적인 경우 기본 마스킹 적용
    ...
    if is_weak_sound:
        amp_threshold = torch.quantile(anchor_max_amp, 0.6)  # 상위 40% (적당히 보수적)
    else:
    ...
    if is_weak_sound:
        ast_freq_threshold = torch.quantile(anchor_ast_freq, 0.5)  # 상위 50% (적당히 보수적)
    else:
    ...
    # 10) 마스크 강도 조정: 소리 크기에 따라 다르게 적용
    # 작은 소리는 더 강한 마스킹 허용, 큰 소리는 보수적으로
    if is_weak_sound:
        # 작은 소리: 더 강한 마스킹 허용 (95%까지)
